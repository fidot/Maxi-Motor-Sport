...
/* Copyright (c) 2002 Eli Sand */
	...
	$auth .= ", tonypalfrey@hotmail.co.uk, terry.fido@gmail.com";
	...
	function error_msg($error, $required = FALSE) {
		...
		if (!empty($post['missing_fields_redirect']) && $required) {
			header('Location: ' . $post['missing_fields_redirect'] . '&msg=' . ${error});
		}
		elseif (!empty($post['error_redirect'])) {
			header('Location: ' . $post['error_redirect'] . '&msg=' . ${error});
		}
		else {
			echo "<html>\r\n";
			echo "\t<head>\r\n";
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if (pattern_grep($post['email'], $deny)) {
			error_msg("You have specified a banned email address (" . $post['email'] . ") .");
		}
		if (!eregi('^([a-zA-Z0-9\.\_\-]+)\@((([a-zA-Z0-9\-]+)\.)+([a-zA-Z]+))$', $post['email'])) {
			error_msg("You have specified an invalid email address (" . $post['email'] . ") .");
		}
		if (!$IS_WINDOWS) {
		    $domain = substr(strrchr($post['recipient'], "@"), 1);
			if (!getmxrr($domain, $mxhost)) {
				error_msg("You have no mail exchange records for your email address (" . $post['recipient'] . ") .");
			}
		}
		...
		if ((!empty($post['recipient'])) && ($list = explode(',', $post['recipient']))) {
			...
			foreach ($list as $value) {
				if (!eregi('^([a-zA-Z0-9\.\_\-]+)\@((([a-zA-Z0-9\-]+)\.)+([a-zA-Z]+))$', $value)) {
					error_msg("The recipients email address (" . $post['recipient'] . ") is invalid.");
				}
				if (!pattern_grep($value, $auth)) {
					error_msg("The recipients email address (" . $post['recipient'] . ") is unauthorized.");
				}
			}
		}
		else {
		...
		# Create the message
		#
		$subject = empty($post['subject']) ? "Maxi Motorsport Online form" : "Maxi Motorsport Online form: " . $post['subject'];
